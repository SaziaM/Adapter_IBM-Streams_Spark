...
#include "BeJwrMSo2SUksSQw1NCzJL0nMCSlPTS0pBgBaogg_1.h"
#include <sstream>
...
{
    instantiators_.insert(std::make_pair("tuple<rstring data,uint64 totalTweets>",&initer));
...
#define MY_OFFSETOF(member, base) \
    ...
    {
        std::string s("totalTweets");
        TypeOffset t(MY_OFFSETOF(totalTweets_, tm), 
...
void SELF::deserialize(std::istream & istr, bool withSuffix)
{
   ...
   if (!readAttributeIdentifier(istr, s)) { return; }
   if (s != "totalTweets") { istr.setstate(std::ios_base::failbit); return; }
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   if (withSuffix)
     SPL::deserializeWithSuffix(istr, totalTweets_);
   else
     istr >> totalTweets_;
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::deserializeWithNanAndInfs(std::istream & istr, bool withSuffix)
{
   ...
   istr >> c; if (!istr) { return; }
   if (c != '=') { istr.setstate(std::ios_base::failbit); return; }
   SPL::deserializeWithNanAndInfs(istr, totalTweets_, withSuffix);
   if (!istr) { return; }  
   istr >> c; if (!istr) { return; }
...
void SELF::serializeWithPrecision(std::ostream & ostr) const
{
    ...
    SPL::serializeWithPrecision(ostr << "totalTweets=", get_totalTweets()) ;
    ostr << '}';
}
