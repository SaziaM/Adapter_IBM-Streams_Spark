// t2s14smackdownWordsi5match


#ifndef BEJWRMSO2NCNOTUZOTSKVZWVPL0OPZJTNTSXJZGAAGZAKAA_H_
#define BEJWRMSO2NCNOTUZOTSKVZWVPL0OPZJTNTSXJZGAAGZAKAA_H_
...
#define SELF BeJwrMSo2NCnOTUzOTskvzwvPL0opzjTNTSxJzgAAgzAKAA
...
public:
    ...
    typedef SPL::rstring smackdownWords_type;
    typedef SPL::int32 match_type;

    enum { num_attributes = 2 } ;
    
    SELF() : Tuple(), smackdownWords_(), match_() {}
    SELF(const Self & ot) : Tuple(), smackdownWords_(ot.smackdownWords_), match_(ot.match_) 
      { constructPayload(ot); }
    SELF(const smackdownWords_type & _smackdownWords, const match_type & _match) : Tuple(), smackdownWords_(_smackdownWords), match_(_match) { }
    ...
    smackdownWords_type & get_smackdownWords() { return smackdownWords_; }
    const smackdownWords_type & get_smackdownWords() const { return smackdownWords_; }
    void set_smackdownWords(const smackdownWords_type & _smackdownWords) { smackdownWords_ = _smackdownWords; }
    
    match_type & get_match() { return match_; }
    const match_type & get_match() const { return match_; }
    void set_match(const match_type & _match) { match_ = _match; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<smackdownWords_type >()(smackdownWords_);
        s = 37 * s + std::tr1::hash<match_type >()(match_);
        return s;
    }
    ...
    size_t getSerializedSize() const
    {
        size_t size = sizeof(SPL::int32);
           size += smackdownWords_.getSerializedSize();
    ...
    { 
        smackdownWords_ = ot.smackdownWords_;
        match_ = ot.match_; 
        assignPayload(ot);
        return *this; 
    }
    ...
    void swap(SELF & ot) 
    { 
        std::swap(smackdownWords_, ot.smackdownWords_);
        std::swap(match_, ot.match_);
...
private:
    ...
    smackdownWords_type smackdownWords_;
    match_type match_;
...
#undef SELF
#endif // BEJWRMSO2NCNOTUZOTSKVZWVPL0OPZJTNTSXJZGAAGZAKAA_H_ 
